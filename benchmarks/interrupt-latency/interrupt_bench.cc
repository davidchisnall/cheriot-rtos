...
void __cheri_compartment("interrupt_bench") entry_high_priority()
{
	...
	if (!headerWritten)
	{
		...
		out.format("#board\tstack size\tlatency\n");
		headerWritten = true;
	}

	int end = CHERI::with_interrupts_disabled([&]() {
		uint32_t bits = 0;
		int ret = event_bits_wait(&t, event_group, &bits, 1, EventWaitAll | EventWaitClearOnExit);
		int time = rdcycle();
...
void __cheri_compartment("interrupt_bench") entry_low_priority()
{
	while(true)
	{
		CHERI::with_interrupts_disabled([]() {
			...
			start = rdcycle();
			yield();
			Debug::log("Low thread after yield");
		});
	}
}
